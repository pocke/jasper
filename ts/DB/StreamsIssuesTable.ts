...
export class StreamsIssuesTable {
  async import(streamId, issues) {
    for (const issue of issues) {
      ...
      const relation = await DB.selectSingle(
        `select * from streams_issues where stream_id = ? and issue_id = ?`,
        params
      );
      if (!relation) {
        await DB.exec(
          "insert into streams_issues (stream_id, issue_id) values (?, ?)",
          params
        );
      }
    }
  ...
  async _unlinkMismatchIssues(issues) {
    ...
    for (const stream of streams) {
      const rows = await DB.select(
        `select issue_id from streams_issues where stream_id = ? and issue_id in (${issueIds})`,
        [stream.id]
      );
      ...
      const targetIssues = issues.filter(issue =>
        targetIssueIds.includes(issue.id)
      );
      ...
      for (const query of queries) {
        mismatchIssues.push(
          ...GitHubQueryParser.takeMismatchIssues(query, targetIssues)
        );
      }
      ...
      if (realMismatchIssues.length) {
        Logger.n(
          `[unlink]: stream: "${stream.name}", queries: "${queries.join(
            ", "
          )}", [${realMismatchIssues.map(v => v.title)}]`
        );
        const mismatchIssueIds = realMismatchIssues
          .map(issue => issue.id)
          .join(",");
        await DB.exec(
          `delete from streams_issues where stream_id = ? and issue_id in (${mismatchIssueIds}) `,
          [stream.id]
        );
      }
    }
