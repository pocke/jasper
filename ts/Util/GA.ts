...
export class GA {
  ...
  async event(category, action, label = null, value = null, session = null) {
    ...
    try {
      ...
      Logger.e(
        `fail sending GA event: category = ${category}, action = ${action}, label = ${label}, value = ${value}, session = ${session}`
      );
      Logger.e(e);
  ...
  async eventAppStart() {
    ...
    const { streamCount } = await DB.selectSingle(
      "select count(1) as streamCount from streams"
    );
    this.event("app", "start/stream", "stream-count", streamCount);

    const { filteredStreamCount } = await DB.selectSingle(
      "select count(1) as filteredStreamCount from filtered_streams"
    );
    this.event(
      "app",
      "start/filter",
    ...
    const { issueCount } = await DB.selectSingle(
      "select count(1) as issueCount from issues"
    );
  ...
  async eventFilteredStreamCreate() {
    const { streamCount } = await DB.selectSingle(
      "select count(1) as streamCount from filtered_streams"
    );
    this.event(
      "filtered-stream",
      "create",
