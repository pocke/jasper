...
export default class FilteredStreamSettingComponent extends React.Component {
  ...
  _show(stream, filter, filteredStream) {
    ...
    if (filteredStream) {
      ...
      dialog.querySelector("#notificationInput").checked =
        filteredStream.notification === 1;
      dialog.querySelector("#colorInput").value = filteredStream.color;
      dialog.querySelector(".icon-flow-cascade").style.color =
        filteredStream.color;
    } else {
  ...
  async _handleOK() {
    ...
    const filter = ReactDOM.findDOMNode(this).querySelector("#filterInput")
      .value;
    ...
    if (name && filter) {
      ...
      if (this._filteredStream) {
        StreamCenter.rewriteFilteredStream(
          this._filteredStream.id,
          name,
          filter,
          notification,
          color
        ...
        await StreamCenter.createFilteredStream(
          this._stream,
          name,
          filter,
          notification,
            ...
            <div>
              ...
              <div className="form-group" title="filter name">
                ...
                <input
                  ...
                  placeholder="filtered stream name"
                />
              </div>
