...
export default class FooterComponent extends React.Component {
  ...
  componentDidMount() {
    {
      let id = SystemStreamEmitter.addUpdateStreamListener(
        this._updateTime.bind(this, "system")
      );
...

    {
      let id = StreamEmitter.addUpdateStreamListener(
        this._updateTime.bind(this, "stream")
      );
  ...
  render() {
    ...
    if (this.state.lastStream) {
      ...
      hoverMessage = `"${
        this.state.lastStream.name
      }" stream connection at ${lastDate}`;
    }
    ...
    return (
      <footer className="toolbar toolbar-footer">
        ...
        <span
          className={`new-version-available ${newVersion ? "" : "hidden"}`}
          onClick={this._handleNewVersion.bind(this)}
        >
          {newVersion}
        </span>
      </footer>
