...
export default class StreamsComponent extends React.Component {
  ...
  componentDidMount() {
...

    {
      ...
      id = SystemStreamEmitter.addRestartAllStreamsListener(
        this._loadStreams.bind(this)
      );
      this._systemStreamListenerIds.push(id);
    }

    {
      ...
      id = StreamEmitter.addSelectStreamListener((stream, filteredStream) => {
        if (filteredStream) {
          this.setState({
            selectedStream: null,
            selectedFilteredStream: filteredStream
          });
        } else {
          this.setState({
      ...
      id = StreamEmitter.addRestartAllStreamsListener(
        this._loadStreams.bind(this)
      );
      this._streamListenerIds.push(id);
    }
  ...
  _setupSorting() {
    ...
    rootEl.addEventListener("mouseup", async evt => {
      ...
      if (underEl) {
        ...
        for (const streamEl of Array.from(
          rootEl.querySelectorAll(".nav-group-item")
        )) {
          if (streamEl === targetEl) continue;

          streamInfoList.push({
            type: streamEl.dataset.streamType,
            id: streamEl.dataset.streamId
          });
    ...
    menu.append(
      new MenuItem({
        ...
        click: () => {
          if (
            confirm(
              `Would you like to mark "${filteredStream.name}" all as read?`
            )
          ) {
  ...
  _renderStreamNodes() {
    const streams = this.state.streams;
    const filteredStreams = this.state.filteredStreams;
    const mixedStreams = [...streams, ...filteredStreams].sort(
      (a, b) => a.position - b.position
    );

    return mixedStreams.map(stream => {
      if (stream.filter) {
        // make node of filteredStream
        const filteredStream = stream;
        const originalStream = streams.find(
          stream => stream.id === filteredStream.stream_id
        );
        const selected =
          this.state.selectedFilteredStream &&
          this.state.selectedFilteredStream.id === filteredStream.id
            ? "active"
            : "";
        ...
        return (
          <a
            key={"filtered-stream-" + filteredStream.id}
            ...
            onClick={this._handleClickWithFilteredStream.bind(
              this,
              filteredStream,
              originalStream
            )}
            onContextMenu={this._handleContextMenuWithFilteredStream.bind(
              this,
              filteredStream,
              originalStream
            )}
          >
            ...
            <span className="stream-unread-count">
              {filteredStream.unreadCount}
            </span>
          </a>
        );
      } else {
        ...
        const selected =
          ...
          this.state.selectedStream.id === stream.id
            ? "active"
            : "";
  ...
  render() {
    return (
      <nav className="nav-group sortable-nav-group">
        <h5 className="nav-group-title">
          <span>STREAMS</span>
          <span
            className="icon icon-plus stream-add"
            onClick={this._handleOpenStreamSetting.bind(this)}
            title="create stream"
          />
        </h5>
