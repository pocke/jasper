...
export default class AppComponent extends React.Component {
  ...
  componentDidMount() {
    {
      let id = SystemStreamEmitter.addUpdateStreamListener(
        this._showNotification.bind(this, "system")
      );
      this._systemStreamListenerId.push(id);
    }

    {
      let id = StreamEmitter.addUpdateStreamListener(
        this._showNotification.bind(this, "stream")
      );
  ...
  async _showNotification(type, streamId, updatedIssueIds) {
    ...
    if (!stream.notification) {
      const tmp = await this._notificationWithFilteredStream(
        streamId,
        updatedIssueIds
      );
    ...
    const targetDate = DateConverter.localToUTCString(
      new Date(Date.now() - 24 * 60 * 60 * 1000)
    ); // 1day ago
    const recentlyIssues = issues.filter(
      issue => issue.updated_at > targetDate
    );
    if (recentlyIssues.length === 0) return;

    // build body
    const title = `"${
      filteredStream ? filteredStream.name : stream.name
    }" was updated (${issues.length})`;
    let body;
    if (issues.length === 1) {
      body = `"${issues[0].title}"`;
    } else {
  ...
  _handleMovingStream(direction) {
    ...
    const activeStreamEl = document.querySelector(
      ".streams-pane.streams .nav-group-item.active"
    );
    if (activeStreamEl) {
      ...
      if (
        targetStreamEl &&
        !targetStreamEl.classList.contains("nav-group-item")
      )
        targetStreamEl = null;

      if (!targetStreamEl) {
        const parentEl =
          activeStreamEl.parentElement[`${prefix}ElementSibling`];
        const querySuffix = direction > 0 ? "first-of-type" : "last-of-type";
        if (parentEl)
          targetStreamEl = parentEl.querySelector(
            `.nav-group-item:${querySuffix}`
          );
      }
    } else {
      targetStreamEl = document.querySelector(
        ".streams-pane.streams .nav-group-item"
      );
    }
  ...
  _handleCommand(commandItem) {
    switch (commandItem.command) {
      ...
      case "load_inbox":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(2) .nav-group-item:nth-of-type(1)"
        );
        break;
      case "load_unread":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(2) .nav-group-item:nth-of-type(2)"
        );
        break;
      case "load_open":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(2) .nav-group-item:nth-of-type(3)"
        );
        break;
      case "load_mark":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(2) .nav-group-item:nth-of-type(4)"
        );
        break;
      case "load_archive":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(2) .nav-group-item:nth-of-type(5)"
        );
        break;
      ...
      case "load_me":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(3) .nav-group-item:nth-of-type(1)"
        );
        break;
      case "load_team":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(3) .nav-group-item:nth-of-type(2)"
        );
        break;
      case "load_watching":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(3) .nav-group-item:nth-of-type(3)"
        );
        break;
      case "load_subscription":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(3) .nav-group-item:nth-of-type(4)"
        );
        break;
      ...
      case "load_1st":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(4) .nav-group-item:nth-of-type(1)"
        );
        break;
      case "load_2nd":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(4) .nav-group-item:nth-of-type(2)"
        );
        break;
      case "load_3rd":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(4) .nav-group-item:nth-of-type(3)"
        );
        break;
      case "load_4th":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(4) .nav-group-item:nth-of-type(4)"
        );
        break;
      case "load_5th":
        this._handleLoadStream(
          ".streams-pane.streams .nav-group:nth-of-type(4) .nav-group-item:nth-of-type(5)"
        );
        break;
    }
  }
      ...
      <div className="window app-window">
        <div className="window-content">
          <div className="pane-group">
            ...
            <div className="pane issues-pane">
              <IssuesComponent />
            </div>
