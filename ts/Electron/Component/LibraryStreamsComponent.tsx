...
export default class LibraryStreamsComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    {
      let id;
      id = LibraryStreamEmitter.addSelectFirstStreamListener(
  ...
  render() {
    ...
    const nodes = this.state.streams.map(stream => {
      const active =
        ...
        this.state.selectedStream.name === stream.name
          ? "active"
          : "";
      ...
      return (
        <a
          key={stream.name}
          className={`nav-group-item ${active} ${unread}`}
