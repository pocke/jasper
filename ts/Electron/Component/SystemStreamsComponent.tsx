...
export default class SystemStreamsComponent extends React.Component {
  ...
  _openSubscribeDialog() {
    const dialog = ReactDOM.findDOMNode(this).querySelector(
  ...
  async _handleSubscriptionOK() {
    ...
    const stream = this.state.streams.find(
      stream => stream.id === SystemStreamCenter.STREAM_ID_SUBSCRIPTION
    );
  ...
  _isIssueUrl(url) {
    ...
    let isIssue = !!url.match(
      new RegExp(`^https://${host}/[\\w\\d-_.]+/[\\w\\d-_.]+/issues/\\d+$`)
    );
    let isPR = !!url.match(
      new RegExp(`^https://${host}/[\\w\\d-_.]+/[\\w\\d-_.]+/pull/\\d+$`)
    );
  ...
  render() {
    ...
    function title(stream) {
      switch (stream.id) {
        case -1:
          return "issues you created, assigned, commented, mentioned or your repository";
        case -2:
          return "issues your team is mentioned in";
        case -3:
          return "issues of repository you watch";
        case -4:
          return "issues you subscribed to";
      }
    }

    const streamNodes = this.state.streams.map(stream => {
      const selected =
        this.state.selectedStream && this.state.selectedStream.id === stream.id
          ? "active"
          : "";
      const enabled = stream.enabled ? "enabled" : "disabled";
      const unread =
        stream.unreadCount > 0 && stream.enabled ? "is-unread" : "";
      return (
        <a
          key={stream.id}
          title={title(stream)}
        ...
        >
          ...
          <span className="stream-unread-count">
            {stream.enabled ? stream.unreadCount : "-"}
          </span>
        </a>
      );
    });
              ...
              <div>
                ...
                <div className="form-actions">
                  <button
                    ...
                    onClick={this._handleSubscriptionCancel.bind(this)}
                  >
                    Cancel
                  </button>
                  <button
                    className="btn btn-form btn-primary"
                    onClick={this._handleSubscriptionOK.bind(this)}
                  >
                    OK
                  </button>
                </div>
