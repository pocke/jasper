...
export class IssueFilter {
  ...
  buildCondition(filterQuery) {
    ...
    } = GitHubQueryParser.parse(filterQuery);
  ...
  _buildPositiveFilterCondition(filterMap) {
    ...
    if (filterMap.is.read)
      conditions.push("(read_at is not null and read_at >= updated_at)");
    if (filterMap.is.unread)
      conditions.push("(read_at is null or read_at < updated_at)");
    ...
    if (filterMap.numbers.length) {
      conditions.push(
        `(number is not null and number in (${filterMap.numbers.join(",")}))`
      );
    }
    ...
    if (filterMap.assignees.length) {
      ...
      const value = filterMap.assignees
        .map(assignee => `assignees like "%<<<<${assignee}>>>>%"`)
        .join(" or ");
      conditions.push(`(${value})`);
    }

    if (filterMap.milestones.length) {
      const value = filterMap.milestones
        .map(milestone => `"${milestone}"`)
        .join(",");
      conditions.push(
        `(milestone is not null and lower(milestone) in (${value}))`
      );
    }
  ...
  _buildNegativeFilterCondition(filterMap) {
    ...
    if (filterMap.numbers.length) {
      conditions.push(
        `number is not null and number not in (${filterMap.numbers.join(",")})`
      );
    }
    ...
    if (filterMap.authors.length) {
      const value = filterMap.authors.map(author => `"${author}"`).join(",");
      conditions.push(
        `(author is not null and lower(author) not in (${value}))`
      );
    }
    ...
    if (filterMap.assignees.length) {
      // hack: assignee format
      const value = filterMap.assignees
        .map(assignee => `assignees not like "%<<<<${assignee}>>>>%"`)
        .join(" and ");
      conditions.push(`(${value})`);
    }
    ...
    if (filterMap.milestones.length) {
      const value = filterMap.milestones
        ...
        .join(",");
      conditions.push(
        `(milestone is not null and lower(milestone) not in (${value}))`
      );
    }
    ...
    if (filterMap.labels.length) {
      // hack: label format
      const value = filterMap.labels
        .map(label => `labels not like "%<<<<${label}>>>>%"`)
        .join(" and ");
      conditions.push(`(${value})`);
    }
  ...
  _buildSortCondition(value) {
    ...
    for (const sortCondition of sortConditions) {
      ...
      switch (column) {
        case "number":
          conditions.push(`number ${order ? order : "desc"}`);
          break;
        case "type":
          conditions.push(`type ${order ? order : "asc"}`);
          break;
        case "read":
          conditions.push(`read_at ${order ? order : "desc"}`);
          break;
        case "updated":
          conditions.push(`updated_at ${order ? order : "desc"}`);
          break;
        case "created":
          conditions.push(`created_at ${order ? order : "desc"}`);
          break;
        case "closed":
          conditions.push(`closed_at ${order ? order : "desc"}`);
          break;
        case "archived":
          conditions.push(`archived_at ${order ? order : "desc"}`);
          break;
        case "star":
          conditions.push(`marked_at ${order ? order : "desc"}`);
          break;
        case "author":
          conditions.push(`author ${order ? order : "asc"}`);
          break;
        case "assignee":
          conditions.push(`assignee ${order ? order : "asc"}`);
          break;
        case "user":
          conditions.push(`user ${order ? order : "asc"}`);
          break;
        case "repo":
          conditions.push(`repo ${order ? order : "asc"}`);
          break;
        case "milestone":
          conditions.push(`milestone ${order ? order : "desc"}`);
          break;
        case "dueon":
