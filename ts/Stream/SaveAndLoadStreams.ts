...
export class SaveAndLoadStreams {
  async save() {
    ...
    const filters = await DB.select(
      "select * from filtered_streams order by stream_id, position"
    );
  ...
  async load(data) {
    let { id: streamIndex, count: streamCount } = await DB.selectSingle(
      "select max(id) + 1 as id, count(1) as count from streams"
    );
    let { id: filterIndex, count: filterCount } = await DB.selectSingle(
      "select max(id) + 1 as id, count(1) as count from filtered_streams"
    );
