  ...
  async _startDB() {
    ...
    // migration to v0.2.1 from v0.2.0
    {
      try {
        ...
        for (const issue of issues) {
          ...
          await DB.exec("update issues set assignees = ? where id = ?", [
            names,
            issue.id
          ]);
        }
    ...
    // migration to v0.3.0 from v0.2.5
    {
      try {
        ...
        for (const issue of issues) {
          const value = JSON.parse(issue.value);
          await DB.exec("update issues set body = ? where id = ?", [
            value.body,
            issue.id
          ]);
        }
    ...
    // migration to v0.4.0 from v0.3.1
    {
      try {
        ...
        await DB.exec(
          "update issues set read_body = body, prev_read_body = body"
        );
    ...
    await DB.exec(
      `create index if not exists read_at_index on issues(read_at)`
    );
    await DB.exec(
      `create index if not exists archived_at_index on issues(archived_at)`
    );
    await DB.exec(
      `create index if not exists marked_at_index on issues(marked_at)`
    );
    await DB.exec(
      `create index if not exists closed_at_index on issues(closed_at)`
    );
    await DB.exec(
      `create index if not exists created_at_index on issues(created_at)`
    );
    await DB.exec(
      `create index if not exists updated_at_index on issues(updated_at)`
    );
    ...
    await DB.exec(
      `create index if not exists assignees_index on issues(assignees)`
    );
    await DB.exec(
      `create index if not exists milestone_index on issues(milestone)`
    );
    ...
    await DB.exec(
      `create index if not exists html_url_index on issues(html_url)`
    );
    await DB.exec(
      `create index if not exists archived_updated_index on issues(archived_at,updated_at)`
    );
    await DB.exec(
      `create index if not exists archived_created_index on issues(archived_at,created_at)`
    );
    await DB.exec(
      `create index if not exists archived_closed_index on issues(archived_at,closed_at)`
    );
    await DB.exec(
      `create index if not exists archived_read_index on issues(archived_at,read_at)`
    );
    await DB.exec(
      `create index if not exists archived_due_index on issues(archived_at,due_on)`
    );
    ...
    const temp = await DB.selectSingle(
      `select count(1) as count from system_streams`
    );
    if (temp.count === 0) {
      await DB.exec(`
        insert into
    ...
    await DB.exec(
      `create index if not exists url_index on subscription_issues(url)`
    );
    ...
    await DB.exec(
      `create index if not exists stream_issue_index on streams_issues(stream_id, issue_id)`
    );
    ...
    await DB.exec(
      `create index if not exists filter_index on filter_histories(filter)`
    );
    await DB.exec(
      `create index if not exists created_at_index on filter_histories(created_at)`
    );
    ...
    await DB.exec(
      `create index if not exists stream_index on filtered_streams(stream_id)`
    );
    await DB.exec(
      `create index if not exists position_index on filtered_streams(position)`
    );
